  ...
  methods: {
    ...
    processFile(event) {
      ...
      // Procedure if supplied file is expected to be a single file.
        ...
        /* TODO: HTML File */
        if (uploader.blueprints[0].format === 'html') {
          if (!files[0].name.endsWith('.html')) {
            let errorMsg = uploader.$t('error-wrong-file-type', 'en', {actualType: files[0].name.substr(files[0].name.lastIndexOf(".")), expectedType: '.html'});
            uploader.postError(4105, errorMsg, uploader.blueprints[0].id);
            uploader.recordError(uploader.$t('error-wrong-file-type', {actualType: files[0].name.substr(files[0].name.lastIndexOf(".")), expectedType: '.html'}), uploader.blueprints[0].id.toString());
          }
        }
    ...
    processContent(content, blueprint) {
      ...
      let extractedData = [];
      let specialParsing = false;

      // let nToExtract = 10;

      // JSON PARSING
      if (blueprint.format === 'json' && blueprint.regex_path != 'chat.html') {
        try {
        ...
        for (let i = 0; i < fileContent.length; i++) {
          ...
          for (let node of Object.values(conversation['mapping'])) {
            if (
              ...
              let msg = node.message.content.parts[0];
              result.push({'sender': author, 'message': msg});
            }
          }
          extractedData.push(result);
          if (i == 9) {
